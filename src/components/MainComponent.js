  ...
  render() { //When we are using (rendering) React components, the syntax will look like HTML or JSX tags with the angle brackets, <>. React components are capitalized, so Navbar and NavbarBarnd are React components.
    return (
      <div>
        ...
        {/*The CampsiteInfo component is expecting an object, not an array (the Filter Method will return an array). To get an Object, and since the selected campsite is the only thing in the array (with index number of 0), we use the index number [0] to get that one campsite object to the CampsiteInfo component.*/}
